...
function InquiryForm() {
  const { admissionsCta } = useSiteContent();
    ...
    return (
      <div className="inquiry-form form-success">
        ...
        <p>
          ...
          <strong>{form.phone}</strong> to discuss the right session for {form.childName}.
        </p>
        <button
          type="button"
        ...
        <select id="program" required value={form.program} onChange={update("program")}>
          ...
          {sessions.map((option) => (
            <option key={option._id || option.title} value={option.title}>
              {option.title} - {option.suitableFor}
            </option>
          ))}
