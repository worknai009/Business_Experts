...
import { positioningQuote, programFormats, programOutcomes } from "../data/perfectManContent";

function HomePage() {
  const { admissionsCta, heroStats, programBrand, programConcept, programImages } = useSiteContent();
    ...
    <main>
      ...
      <section className="services-section">
        <div className="section-heading">
          <p className="eyebrow">Program Sessions</p>
          <h2>Flexible Sessions for Every Schedule</h2>
          <p className="section-copy">
            Daily sessions, weekends, a crash program, or a complete transformation journey - pick
        ...
        <div className="program-table-wrap home-session-table">
          <table className="program-table">
            ...
            <tbody>
              {sessions.slice(0, 4).map((session) => (
                <tr key={session._id || session.title}>
                  <td>{session.title}</td>
                  <td>{session.duration}</td>
                  <td>{session.suitableFor}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="hero-actions center-actions">
          <Link to="/programs" className="primary-btn">
            Explore All Sessions <ArrowRight size={17} />
          </Link>
        </div>
      </section>
      ...
      <section className="admissions-section">
        <div className="admissions-card">
          ...
          <p className="section-copy">
            Explore our sessions and training modules, or book a Child Assessment today.
          </p>
          <div className="hero-actions">
